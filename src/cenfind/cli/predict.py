...
def run(args):
    ...
    for field, channel in dataset.pairs('test'):
        _, foci = extract_foci(field, args.model, channel, prob_threshold=.5)
        foci = [Centre((r, c), f_id, 'Centriole') for f_id, (r, c) in enumerate(foci)]
        print( "Writing visualisations for field: %s, channel: %s, %s foci detected" % (field.name, channel, len(foci)))
        vis = save_visualisation(field, foci, channel, channel_nuclei=args.channel_nuclei)
        tf.imwrite(
            model_run / f"{field.name}_C{channel}_pred.png", vis
        )
