...
def register_parser(parent_subparsers):
    parser = parent_subparsers.add_parser("train", help="Train a Spotnet model using the datasets")
    parser.add_argument('datasets', type=Path, help='Path to the dataset')
    parser.add_argument('--model_path', type=Path, required=True, help='Path to the model fit')
...
def run(args):
    ...
    model_multiscale = SpotNet(config_multiscale, name=time_stamp, basedir=args.model_path)

    model_multiscale.train(all_train_x, all_train_y, validation_data=(all_test_x, all_test_y), epochs=args.epochs)
